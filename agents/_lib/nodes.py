    ...
    if _question_model_cache is None:
        ...
        _question_model_cache = base.bind_tools([{
            ...
            "function": {
                ...
                "parameters": GeneratedQuestion.model_json_schema(),
            },
        }])
...
def generate_question(state: QuizState) -> dict:
    ...
    # bind_tools returns AIMessage with tool_calls; parse manually
    tool_calls = getattr(result, "tool_calls", None) or []
    if not tool_calls:
        raise RuntimeError("LLM did not return a tool call with question data")
    parsed = GeneratedQuestion.model_validate(tool_calls[0].get("args", {}))
    ...
    correct = parsed.correct_option.strip().upper()[:1]
